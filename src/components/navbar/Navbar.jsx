import React, { useState } from 'react';
import { RiMenu3Line, RiCloseLine } from 'react-icons/ri';
import logo from '../../assets/logo.svg'; 
import './navbar.css';

const Navbar = () => {
  const [toggleMenu, setToggleMenu] = useState(false);

  return (
    <div className="trade__navbar">
      <div className="trade__navbar-links">
        <div className="trade__navbar-links_logo">
          <img src={logo} alt='logo'/>
        </div>
        <div className="trade__navbar-links_container">
          <p><a href="#home">Home</a></p>
          <p><a href="#wtrade">About</a></p>
          <p><a href="#possibility">Web3.0</a></p>
          <p><a href="#features">Contact Us</a></p>
          <p><a href="#blog">Portfolio</a></p>
        </div>
      </div>
      <div className="trade__navbar-menu">
        {toggleMenu
          ? <RiCloseLine color="#fff" size={27} onClick={() => setToggleMenu(false)} />
          : <RiMenu3Line color="#fff" size={27} onClick={() => setToggleMenu(true)} />}
        {toggleMenu && (
        <div className="trade__navbar-menu_container scale-up-center">
          <div className="trade__navbar-menu_container-links">
          <p><a href="#home">Home</a></p>
          <p><a href="#wtrade">About</a></p>
          <p><a href="#possibility">Web3.0</a></p>
          <p><a href="#features">Contact Us</a></p>
          <p><a href="#blog">Portfolio</a></p>
          </div>
        </div>
        )}
      </div>
    </div>
  );
};

export default Navbar;